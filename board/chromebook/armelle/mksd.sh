...
imagesec=$((2*headersec+kernelsec+efisec+rootfssec))
...
efiblocks=$((efisec<<4))
efistart=$((kernelstart+kernelblocks))
rootfsblocks=$((rootfssec<<4))
rootfsstart=$((efistart+efiblocks))
...
run $cgpt add -i 2 -b $efistart -s $efiblocks \
	-t efi -l efi $bootsd
...
run dd bs=$block if=$efi of=$bootsd seek=$efistart conv=notrunc
