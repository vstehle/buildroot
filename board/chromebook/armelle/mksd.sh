...
imagesec=$((2*headersec+kernelsec+bootsec+rootfssec))
...
bootblocks=$((bootsec<<4))
bootstart=$((kernelstart+kernelblocks))
rootfsblocks=$((rootfssec<<4))
rootfsstart=$((bootstart+bootblocks))
...
run $cgpt add -i 2 -b $bootstart -s $bootblocks \
	-t data -l boot $bootsd
run $sgdisk -A 2:set:2 $bootsd
...
run $cgpt add -i 3 -b $rootfsstart -s $rootfsblocks \
	-t data -l rootfs $bootsd

run dd bs=$block if=$kernel of=$bootsd seek=$kernelstart conv=notrunc
run dd bs=$block if=$boot of=$bootsd seek=$bootstart conv=notrunc
run dd bs=$block if=$rootfs of=$bootsd seek=$rootfsstart conv=notrunc
